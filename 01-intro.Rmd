...
```{r fgintro01,warning=FALSE, message=FALSE, echo = FALSE, fig.align='center',fig.cap = "Quy trình áp dụng Khoa học dữ liệu để giải quyết một vấn đề thực tế"}
a<-0.1
x<-0.3
y<-0.25
z<-0.05
...
M[6,]<-c(a+3*y+z, a+x)
par(mar=c(2,0,2,0))
plotmat(A, pos = M, box.type = "square", box.prop = 0.4, box.col = c(rep("lightblue",2),rep("orange",3),"pink"),
        name = c("Nhập liệu", "Sắp xếp", "Biến đổi", "Trực quan", "Mô hình", "Kết quả" ), curve = 0,
        shadow.size = 0.0, arr.type = "curved", box.size = 0.08, arr.col = "grey",arr.pos = 0.5, main = "Quy trình áp dụng Khoa học dữ liệu vào giải quyết vấn đề")
