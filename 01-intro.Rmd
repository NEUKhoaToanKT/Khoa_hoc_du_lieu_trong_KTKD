...
```{r fgintro01, warning=FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.cap = "Quy trình áp dụng Khoa học dữ liệu để giải quyết một vấn đề thực tế", fig.retina=5}
a<-0.1
x<-0.3
y<-0.25
z<-0.05
...
M[6,]<-c(a+3*y+z, a+x)
par(mar=c(2,0,2,0))
plotmat(A, pos = M, box.type = "square", box.prop = 0.4, box.col = c(rep("white",2),rep("white",3),"white"),
        name = c("Nhập liệu", "Sắp xếp", "Biến đổi", "Trực quan", "Mô hình", "Kết quả" ), curve = 0, txt.col = "#640514",
        shadow.size = 0.0, arr.type = "curved", box.size = 0.08, arr.col = "grey",arr.pos = 0.5, main = "Quy trình áp dụng KHDL vào giải quyết vấn đề thực tế")

rect(0, a/2+x-0.05, a+y/2-0.028, a+2*x+a/2+0.05, col = "grey", density = 0)
rect(a+y-0.05 ,0.0 ,a+2*y+y/2+0.02, a+2*x+a/2+0.05, col = "grey", density = 0)
straightarrow(c(a+2*y+0.145,a+x), c(a+2*y+0.22,a+x), col = "grey",arr.col = "grey",arr.pos = 0.5)
textrect(c(a, a + 3*x-0.08), 0.1, 0.05,lab = "Tiền xử lý", box.col = "white",
          shadow.size = 0, cex = 1.5,lcol = "white")
textrect(c(a + y + y/2+0.04, a + 3*x-0.08), 0.1, 0.05,lab = "Hiểu về dữ liệu", box.col = "white",
          shadow.size = 0, cex = 1.5,lcol = "white")
...
p4<-dat%>%ggplot(aes(y=Social_Media,x=TV))+geom_point(shape=21,color="#640514")+
  geom_smooth(method="lm", se=FALSE, size = 0.5, color = "darkblue")+
  theme_classic()+ylab("Qc mạng xã hội (triệu)")+xlab("Quảng cáo TV (triệu)")

p5<-dat%>%ggplot(aes(y=Flyer,x=TV))+geom_point(shape=21,color="#640514")+
  geom_smooth(method="lm", se=FALSE, size = 0.5, color = "darkblue", )+
  theme_classic()+ylab("Qc tờ rơi (triệu)")+xlab("Quảng cáo TV (triệu)")

p6<-dat%>%ggplot(aes(y=Flyer,x=Social_Media))+geom_point(shape=21,color="#640514")+
  geom_smooth(method="lm", se=FALSE, size = 0.5, color = "darkblue")+
  theme_classic()+ylab("Qc tờ rơi (triệu)")+xlab("Qc mạng xã hội (triệu)")
...
```{r fgintro02, warning=FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.cap = "Quá trình phát triển của các mô hình học máy", fig.retina=5}
par(mar=c(2,0,2,0))
plot.new()
#par(fig=c(0,0.5,0,1))
#par(mar=c(0,0,0,0)) # (2,0,2,0)
names=c("TK19","1940","1970","1980","1990","2000","2010","2020")
straightarrow(c(0,0.5), c(1,0.5),arr.pos =1, lcol = "black",arr.type = "triangle", lwd = 1, arr.length = 0.5,arr.width = 0.4, arr.col = "#640514")
#box(col="grey")
d=0.9/12
N <- matrix(nrow = 13, ncol = 2, byrow = TRUE, data = 0)
N[1,]= c(0.1,0.5)
x=c()
x=c(x,0.1)
N[2,] = c(0.1+2*d,0.5)
x=c(x,0.1+2*d)
for (i in 4:9){
  N[i-1,]= c(0.1+ d*(i+1),0.5)
  x=c(x,0.1+(i+1)*d)
}
for (i in (1:8)){
  en.Node(N[i,],pos= 2.5,cex=0.8,col = "#640514")
  k= ifelse(i>2, i+2, ifelse(i==2,2,0))
  quynh=ifelse(i%%2==0,0.55,0.45)
  text(0.1+k*d,quynh, names[i], font = 3, cex = 1, col = "#640514")
  len=ifelse(i%%2==0,0.15,0.85)
  straightarrow(N[i,],c(x[i],len) ,arr.pos =1, arr.type = "circle", lwd = 0.05, arr.length = 0.05, arr.width = 0.02 ,arr.col = "#640514",cex = 0.1)
}
len=0.97
xuo=0.05
hi = 0.08 #dãn dòng
size_text=0.8
textplain(mid = c(0.1, len), adj = c(0.5, 0.5), font = 3, height = hi, lab = c("Mô hình","hồi quy"),cex=size_text)
textplain(mid = c(x[2], xuo), adj = c(0.5, 0.5), font = 3, height = hi, lab = c("Hồi quy","logistic"),cex=size_text)
textplain(mid = c(x[3], len), adj = c(0.5, 0.5), font = 3, height = 0.1, lab = c("Mô hình","tuyến tính","tổng quát"),cex=size_text)
textplain(mid = c(x[4], xuo), adj = c(0.5, -0.5), font = 3, height = hi, lab = c("Cây","quyết định"),cex=size_text)
textplain(mid = c(x[5], len), adj = c(0.5, 0.5), font = 3, height = 0.1, lab = c("Mô hình","mạng", "Nơ-ron"),cex=size_text)
textplain(mid = c(x[6], xuo), adj = c(0.4, 0.5), font = 3, height = 0.1, lab = c("Rừng ngẫu nhiên","và","học tăng cường"),cex=size_text)
textplain(mid = c(x[7], len), adj = c(0.5, 0.5), font = 3, height = hi, lab = c("XGB","+LightGBM"),cex=size_text)
textplain(mid = c(x[8], xuo), adj = c(0, 0), font = 3, height = hi, lab = c("Học sâu"),cex=size_text)
```
