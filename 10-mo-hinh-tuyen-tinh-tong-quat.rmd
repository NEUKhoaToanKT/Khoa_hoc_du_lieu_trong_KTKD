...
Thay vì cố gắng hiểu các khái niệm toán học ở trên, chúng ta hãy thử áp dụng mô hình kể trên trong một trường hợp cụ thể. Chúng ta sẽ xây dựng một mô hình tuyến tính tổng quát mà trong đó biến mục tiêu $Y$ cho biết người mua bảo hiểm xe có hay không lựa chọn các quyền lợi bảo hiểm bổ sung khi ký hợp đồng mua bảo hiểm trách nhiệm dân sự bắt buộc. Dữ liệu được sử dụng có tên là `r colorize_style("MotoInsurance")` bao gồm có 6 biến độc lập là:

- 1. Độ tuổi của người lái xe, biến `r colorize_style("age")`, nhận giá trị là các số nguyên dương từ 15 đến 92 tuổi.
- 2. Kinh nghiệm lái xe, biến `r colorize_style("seniority")`, cho biết số năm kinh nghiệm lái xe, giá trị là các số nguyên từ 2 đến 40 năm.
- 3. Giới tính của người lái xe, biến `r colorize_style("sex")`, nhận giá trị `r colorize_style("\\'M\\'")` nếu người lái xe là nam giới và `r colorize_style("\\'F\\'")` nếu người lái xe là nữ giới.
- 4. Nơi xe được đăng ký, biến `r colorize_style("urban")`, nhận giá trị là 1 nếu xe được đăng ký tại khu vực thành phố và nhận giá trị bằng 0 trong các trường hợp còn lại.
- 5. Loại hình đăng ký xe, biến `r colorize_style("private")`, nhận giá trị là 1 nếu xe mua bảo hiểm là xe đăng ký theo cá nhân và nhận giá trị 0 trong các trường hợp còn lại.
- 6. Tình trạng hôn nhân của người lái xe, biến `r colorize_style("marital")`, nhận giá trị `r colorize_style("\\'C\\'")` nếu đã kết hôn, `r colorize_style("\\'S\\'")` tương ứng với chưa kết hôn, và `r colorize_style("\\'O\\'")` tương ứng với đã ly dị.

Biến mục tiêu hay biến phụ thuộc là biến $Y$ nhận một trong hai giá trị, `r colorize_style("\\'Yes\\'")` nếu người mua bảo hiểm trách nhiệm dân sự đồng ý lựa thêm quyền lợi bảo hiểm bổ sung và `r colorize_style("\\'No\\'")` nếu người mua bảo hiểm trách nhiệm dân sự không lựa chọn mua quyền lợi bổ sung. Để mô hình ở dạng đơn giản nhất có thể, chúng tôi lựa chọn hai biến độc lập để xây dựng mô hình là biến `r colorize_style("age")` và biến `r colorize_style("sex")`. Chúng ta sẽ sử dụng biến `r colorize_style("age")` như một biến kiểu số, trong khi biến `r colorize_style("sex")` là một biến kiểu phân loại/rời rạc nhận một trong hai giá trị là `r colorize_style("\\'M\\'")` tương ứng với nam giới và `r colorize_style("\\'F\\'")` tương ứng với nữ giới.

Đoạn lệnh R dưới đây được sử dụng để lấy dữ liệu và phân tích nhanh ảnh hưởng của các biến `r colorize_style("age")` và `r colorize_style("sex")` lên quyết định mua bảo hiểm bổ sung của người sở hữu xe ô tô.

```{r fgglm001a, warning=FALSE, message=FALSE, fig.align='center', fig.retina= 5, fig.cap= "Mối liên hệ giữa các biến giải thích là độ tuổi và giới tính lên biến mục tiêu quyết định mua bảo hiểm bổ sung trong dữ liệu MotoInsurance. Hình bên trái là đồ thị hình hộp cho biến phân phối xác suất của độ tuổi cho nhóm người có mua và không mua. Hình bên phải là đồ thị dạng cột cho biết mối liên hệ giữa giới tính đến quyết định mua bảo hiểm bổ sung"}
...
Từ Hình \@ref(fig:fgglm001a) cho thấy cả hai biến `r colorize_style("age")` và `r colorize_style("sex")` có ảnh hưởng đến quyết định mua bảo hiểm bổ sung. Đồ thị bên trái cho thấy phân phối xác suất của những người có mua bảo hiểm bổ sung thấp hơn, điều này có nghĩa là những người trẻ hơn có xu hướng đồng ý mua bảo hiểm bổ sung hơn. Đồ thị bên phải cho thấy tỷ lệ nữ giới có mua bảo hiểm cao hơn so với tỷ lệ nam giới, hay nói cách khác giới tính nữ có khả năng đồng ý mua bảo hiểm bổ sung cao hơn so với nam giới.

Chúng ta có thể khẳng định mối liên hệ giữa biến `r colorize_style("age")` đến biến mục tiêu $Y$ qua phân tích phương sai:
```{r, warning=FALSE, message=FALSE}
summary(aov(age~Y,data = dat))
```

Mối liên hệ giữa biến giới tính đến biến mục tiêu được khẳng định qua kiểm định `r colorize_style("chisq.test")`
```{r, warning=FALSE, message=FALSE}
chisq.test(dat$Y, dat$sex)
```

Chúng ta sẽ xây dựng một mô hình tuyến tính tổng quát để xác nhận lại các phân tích ở trên và lượng hóa được ảnh hưởng của các biến `r colorize_style("age")` và `r colorize_style("sex")` lên quyết định mua bảo hiểm bổ sung. Các thành phần của mô hình tuyến tính tổng quát mà chúng ta đã đề cập trong phần trên có thể được cụ thể hóa trên dữ liệu `r colorize_style("MotoInsurance")` như sau

* Thứ nhất: biến mục tiêu $Y$ chỉ nhận một trong hai giá trị là `r colorize_style("\\'Yes\\'")` hoặc `r colorize_style("\\'No\\'")` do đó phân phối xác suất cho biến mục tiêu được lựa chọn để xây dựng mô hình là phân phối nhị thức với trung bình là $\rho$, ký hiệu $Y \sim  \mathcal{B}(\rho)$. Giá trị trung bình $\rho$ phụ thuộc vào các biến giải thích.

* Thứ hai: với mỗi quan sát của các biến giải thích `r colorize_style("age")` và `r colorize_style("sex")`, giả sử là `r colorize_style("$age_i$")` và `r colorize_style("$sex_i$")`, chúng ta có giá trị trung bình của phân phối nhị thức là $\rho_i$. Đồng thời mối liên hệ giữa giá trị trung bình $\rho_i$ với các biến giải thích được mô tả thông qua một hàm số được gọi là hàm `r colorize_style("probit")`. Hàm `r colorize_style("probit")` là một hàm số đơn điệu tăng có miền xác định là khoảng (0,1) và nhận giá trị trên tập các số thực.  
...
Giá trị $\rho_i$ là vừa là giá trị trung bình, vừa là xác suất mà người $i$ có tuổi bằng `r colorize_style("$age_i$")`, giới tính `r colorize_style("$sex_i$")`, có mua bảo hiểm bổ sung. Hàm `r colorize_style("probit")` ở trên là hàm ngược của hàm phân phối xác suất của biến ngẫu nhiên phân phối chuẩn có trung bình 0 và phương sai bằng 1. Hàm số này dùng để kết nối giữa giá trị trung bình $\rho_i$ với các giá trị tương ứng của biến giải thích `r colorize_style("$age_i$")` và `r colorize_style("$sex_i$")` và còn được gọi là *hàm liên kết*.

Trong mô hình tuyến tính tổng quát được mô tả trong hệ phương trình \@ref(eq:glm2) có ba tham số cần được ước lượng từ dữ liệu là $\beta_0$, $\beta_1$ và $\beta_2$. Hàm số dùng để xây dựng và ước lượng mô hình tuyến tính tổng quát trong R là hàm `r colorize_style("glm()")` của thư viện `r colorize_style("stats")`. Chúng ta ước lượng các hệ số tuyến tính như sau:
...
Có thể thấy rằng các hệ số của biến `r colorize_style("age")` và `r colorize_style("sex")` đều có ý nghĩa thống kê khi các giá trị *p-value* đều rất nhỏ. Đúng như chúng nhận định từ phần phân tích khám phá dữ liệu, hệ số tuyến tính của biến `r colorize_style("age")` là số âm, bằng -0.016, cho biết người trẻ tuổi hơn có khả năng đồng ý mua bảo hiểm bổ sung cao hơn. Hệ số tương ứng với biến giới tính nam là số âm, bằng -0.447, điều này cho biết khả năng nam giới đồng ý mua bảo hiểm bổ sung là thấp hơn so với nữ giới. Các kết luận có thể đưa ra từ kết quả của mô hình tuyến tính tổng quát ở trên như sau:
...
trong đó `r colorize_style("$age_i$")` là tuổi của người $i$; `r colorize_style("$sex_i$")` là giới tính của người $i$, nhận giá trị bằng 1 nếu người đó là nam giới và 0 nếu người đó là nữ giới. Hàm `r colorize_style("probit")` được lựa chọn trong công thức ở trên là hàm số ngược của hàm phân phối của biến ngẫu nhiên phân phối chuẩn $\mathcal{N}(0,1)$ nên có thể viết mối liên hệ giữa $\rho_i$ và các biến giải thích trong phương trình \@ref(eq:glm3) như sau:
\begin{align}
...
Hàm `r colorize_style("probit")` được lựa chọn làm hàm liên kết vì miền xác định của hàm số này là khoảng (0,1) phù hợp với việc mô tả xác suất $\rho_i$ trong khi miền giá trị của hàm số này là tập các số thực phù hợp với tổ hợp tuyến tính của các biến giải thích.
...
* Thứ hai: mối liên hệ giữa giá trị trung bình của biến mục tiêu $Y$ và tổ hợp tuyến tính của các biến độc lập được thể hiện thông qua một hàm số, trong trường hợp này là hàm `r colorize_style("probit")`. Trong mô hình tuyến tính thông thường, giá trị trung bình của biến mục tiêu bằng tổ hợp tuyến tính của các biến độc lập.
...
```{r fgglm001b,warning=FALSE,message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap = "Các hàm phân phối xác suất thường được sử dụng làm hàm ngược của hàm liên kết khi biến phụ thuộc có phân phối nhị thức. Hình bên trái: các hàm phân phối xác suất. Hình bên phải: các hàm liên kết"}
...
Hàm số `r colorize_style("glm()")` của thư viện `r colorize_style("stats")` được sử dụng để xây dựng mô hình tuyến tính tổng quát. Tham số `r colorize_style("family")` trong hàm `r colorize_style("glm()")` dùng để khai báo phân phối xác suất cho biến phụ thuộc $Y$ và để lựa chọn hàm liên kết phù hợp. Trở lại với ví dụ khi $Y$ là biến nhị phân mô tả khách hàng có hay không lựa chọn các quyền lợi đầy đủ khi tham gia bảo hiểm bổ sung, chúng ra thực hiện xây dựng mô hình như sau
...
```{r glmtab001, echo = FALSE, tab.cap = "Xác suất người bảo hiểm lựa chọn quyền lợi bổ sung tính toán dựa trên mô hình GLM với hai biến giải thích là độ tuổi và giới tính. Các hàm liên kết được lựa chọn bao gồm hàm logit, hàm probit và hàm cauchit"}
kable(dat, booktabs = T, "html",
      #caption = "Xác suất lựa chọn quyền lợi bảo hiểm bổ sung",
      col.names = c("Người BH", "Độ tuổi",
                    "Giới tính", " Logit ",
                    " Probit ", "Cauchit"),align = 'c') %>%
...
Để minh họa cho xây dựng mô hình tuyến tính tổng quát trong trường hợp $Y$ là biến rời rạc danh nghĩa, chúng ta sẽ xây dựng mô hình phân loại trên dữ liệu `r colorize_style("Travel_Insurance")`. Đây là dữ liệu cung cấp thông tin về các sản phẩm bảo hiểm du lịch từ một đại lý du lịch, với biến mục tiêu là `r colorize_style("product.name")` cho biết khách hàng đã lựa chọn sản phẩm sản phẩm bảo hiểm nào. Biến mục tiêu có bốn giá trị danh nghĩa tương ứng với bốn loại sản phẩm là `r colorize_style("Basic Plan")`, `r colorize_style("Bronze Plan")`, `r colorize_style("Silver Plan")`, và `r colorize_style("Value Plan")`. Mặc dù các sản phẩm này có mức giá khác nhau nhưng việc so sánh các giá trị là không có nhiều ý nghĩa. Dữ liệu có 10 biến độc lập, tuy nhiên, để đơn giản hóa, chúng ta chỉ lấy hai biến là giới tính và độ tuổi của người tham gia bảo hiểm. Mối liên hệ giữa độ tuổi của khách hàng và loại sản phẩm khách hàng lựa chọn được mô tả bằng đồ thị dạng hộp như Hình \@ref(fig:fgglm001c).

```{r, warning=FALSE, message=FALSE, echo = FALSE}
# Fit multinomial logistic regression model 
dat<-read.csv("../KHDL_KTKD Final/Dataset/travel insurance.csv")
...
```{r fgglm001c, warning=FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap = "Mối liên hệ giữa giới tính và loại hình bảo hiểm du lịch mà khách hàng lựa chọn"}
# Thực hiện các phân tích khai phá
dat2<-dat1%>%group_by(Product.Name,Gender)%>%summarise(col1 = length(Product.Name))%>%as.data.frame()
dat20<-dat1%>%group_by(Product.Name)%>%summarise(col1 = length(Product.Name))%>%as.data.frame()
dat2<-dat2%>%mutate(total = dat20$col1[match(dat2$Product.Name,dat20$Product.Name)])%>%mutate(Gender_Rate = paste0(round(col1/total*100,0),"%"))
dat2<-dat2%>%arrange(Product.Name,desc(Gender))

dat1%>%ggplot()+geom_bar(aes(x = Product.Name, fill = Gender),alpha = 0.75,col = "#640514")+
  geom_label(data=data.frame(x=c(1,1,2,2,3,3,4,4),
                             y=c(1550,4280,
                                 800,2770,
                                 500, 1600,
                                 800,2000),
                             z = dat2$Gender_Rate),aes(x,y,label = z),col = "#640514",
             size = 4.1)+
  ggtitle("Mối liên hệ giữa giới tính và sản phẩm lựa chọn")+
  theme_minimal()+ylab("Số lượng khách hàng")+xlab("Loại sản phẩm")+
  guides(fill=guide_legend(title="Giới tính"))
  
```

Từ Hình \@ref(fig:fgglm001c) có thể thấy rằng Nam giới có xu hướng lựa chọn các loại hình bảo hiểm du lịch `r colorize_style("Basic Plan")` và `r colorize_style("Value Plan")` khi tỷ lệ Nam lựa chọn mỗi loại Hình bảo hiểm này lần lượt là 58% và 66%. Nữ giới có xu hướng lựa chọn `r colorize_style("Bronze Plan")`, `r colorize_style("Silver Plan")` khi tỷ lệ Nữ giới lựa chọn là 58% và 56%. Có thể kết luận được rằng có khác biệt giữa Nam và Nữ khi lựa chọn loại hình bảo hiểm du lịch
...
```{r fgglm001d, warning=FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.cap = "Mối liên hệ giữa giới tính và loại hình bảo hiểm du lịch mà khách hàng lựa chọn"}
dat1%>%ggplot()+geom_boxplot(aes(x = reorder(Product.Name,Age),
                                 y = Age, fill = Product.Name), 
                             alpha = 0.75,
                             color = "#640514")+
  ggtitle("Mối liên hệ giữa độ tuổi và sản phẩm lựa chọn")+
  theme_minimal()+xlab("Loại sản phẩm")+ylab("Độ tuổi")+
  theme(legend.position = "none")

```

Có thể thấy rằng có sự khác biệt về phân phối của độ tuổi của những người lựa chọn các sản phẩm bảo hiểm: những người trẻ tuổi hơn có xu hướng lựa chọn `r colorize_style("Bronze Plan")`, `r colorize_style("Silver Plan")` trong khi những người lớn tuổi hơn có xu hướng lựa chọn `r colorize_style("Basic Plan")` và `r colorize_style("Value Plan")`.

Từ các phân tích ở trên có thể thấy rằng có sự tác động của biến giới tính và độ tuổi lên biến mục tiêu. Để xây dựng mô hình tuyến tính tổng quát cho biến phân loại danh nghĩa, chúng ta sử dụng hàm `r colorize_style("multinom()")` từ thư viện `r colorize_style("nnet")`:
...
* Với sản phẩm `r colorize_style("Basic Plan")`
...
```{r glmtab002, echo = FALSE, tab.cap = "Xác suất lựa chọn loại hình bảo hiểm du lịch ước tính theo hai biến giải thích là giới tính và độ tuổi của người mua bảo hiểm"}
kable(M2, booktabs = T, "html",
      #caption = "Xác suất lựa chọn sản phẩm bảo hiểm du lịch",
      col.names = c("Khách hàng", "Độ tuổi",
                    "Giới tính", "Bronze Plan","Silver Plan", "Value Plan", "Basic Plan"),align = 'c') %>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```

Kết quả từ mô hình cho thấy với các độ tuổi như trong Bảng \@ref(tab:glmtab002) chỉ có `r colorize_style("Bronze Plan")` và `r colorize_style("Basic Plan")` được lựa chọn nếu chúng ta chỉ sử dụng hai biến độc lập là giới tính và độ tuổi. Đây là kết quả chưa chính xác vì mô hình chỉ sử dụng hai biến giới thích là độ tuổi và giới tính. Lựa chọn của khách hàng còn phụ thuộc vào nhiều yếu tố khác. Chúng tôi muốn nhấn mạnh về cách xây dựng mô hình cho biến phân loại hơn là cố gắng xây dựng một mô hình có khả năng dự báo tốt. Chúng ta sẽ thảo luận về đánh giá hiệu quả của mô hình ở các phần sau của chương sách.
...
Chúng ta sử dụng hàm `r colorize_style("glm()")` để xây dựng và ước lượng mô hình tuyến tính tổng quát:
...
df$Gender<-ifelse(df$Gender=="Male","Nam","Nữ")
...
Ngoài các hệ số của các biến độc lập trong mô hình tuyến tính tổng quát, hàm `r colorize_style("glm()")` còn cung cấp giá trị ước lượng được cho tham số $\phi$ (disperson paramter hay tham số phân tán) là $0.924$. Tham số phân tán được định nghĩa trong họ các phân phối kiểu mũ sẽ được thảo luận trong phần tiếp theo. Đối với phân phối $gamma$, tham số phân tán được tính bằng $\phi = 1/\alpha$. 
...
Hàm `r colorize_style("glm()")` sử dụng xuyên suốt trong chương sách cho phép bạn đọc ước lượng mô hình tuyến tính tổng quát cho đa số các phân phối thường gặp của $Y_i$. Tham số $weight$ trong hàm `r colorize_style("glm()")` là véc-tơ tham số $w_i$ như chúng ta đã trình bày ở trên. Lựa chọn giá trị cho $w_i$ hoàn toàn do cách tiếp cận của người xây dựng mô hình.
...
Kết quả ước lượng khi sử dụng hàm `r colorize_style("glm()")` trong R hiển thị hai giá trị là Null deviance và Residual deviance. Null deviance được tính toán với giả thiết là chỉ có hệ số chặn $\beta_0$, còn Residual deviance được tính toán với véc-tơ $\boldsymbol{\beta}$ đầy đủ.
...
kable(dat, booktabs = T,
  ...
  kable_styling(latex_options = "scale_down",full_width = F)
```
Mô hình $M_1$ và $M_2$ đều chỉ có một biến độc lập và $M_1$ có deviance nhỏ hơn $M_2$ do đó mô hình $M_1$ tốt hơn $M_2$. So sánh mô hình $M_1$ với 1 biến độc lập là `r colorize_style("age")` với mô hình $M_3$ có 2 biến độc lập là `r colorize_style("age")` và $Gender$ cần dựa trên kiểm định $\chi^2$. Ta có $D^{*}(y, \theta^{M_1}) - D^{*}(y, \theta^{M_2}) = 8324.2 - 8316.8 = 7.4$. Giá trị 7.4 tương ứng với mức xác suất $0.994$ của phân phối $\chi^2(1)$. Điều này có ý nghĩa là ở mức độ tin cậy 99\%, có thể kết luận rằng thêm biến $Gender$ vào mô hình $M_1$ là có ý nghĩa thống kê, tuy nhiên việc thêm biến $Gender$ lại không có ý nghĩa thống kê ở mức độ tin cậy $99.5\%$.
...
- Ví dụ 1: chúng ta quay trở lại dữ liệu "MotoInsurance.csv" khi biến mục tiêu $Y$ chỉ nhận hai giá trị là `r colorize_style("\\'No\\'")` hoặc `r colorize_style("\\'Yes\\'")`. Biến $Y$ có thể là biến dạng nhị phân hoặc vừa có thể là biến dạng đếm nếu chúng ta cho tương đương giá trị `r colorize_style("\\'No\\'")` tương đương với 0 và giá trị `r colorize_style("\\'Yes\\'")` tương ứng với 1. Khi xây dựng mô hình tuyến tính tổng quát, chúng ta có thể sử dụng phân phối Poisson cho biến mục tiêu với hàm liên kết $g(\cdot) = log(\cdot)$.
```{r, warning=FALSE, message=FALSE}
dat<-read.csv("../KHDL_KTKD Final/Dataset/MotoInsurance.csv")
