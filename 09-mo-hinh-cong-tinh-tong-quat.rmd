...
```{r fggam01, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5,  fig.width=8, fig.cap="Hồi quy đa thức biến giá nhà (nghìn USD) theo biến tỷ lệ người sống dưới mức trung bình (%) trên dữ liệu Boston. Hình bên trái: sử dụng đa thức bậc ba. Hình ở giữa: sử dụng đa thức bậc bốn. Hình bên phải: sử dụng đa thức bậc năm"}
library(MASS)
dat<-Boston
p1<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(method= "lm",formula = y ~ poly(x,degree = 3, raw = TRUE), size = 0.5, color = "darkblue")+theme_minimal()+ggtitle("Hàm bậc 3")+
  geom_vline(xintercept = 25, col = "black",linetype=2)
p2<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(method= "lm",formula = y ~ poly(x,degree = 4, raw = TRUE), size = 0.5, color = "darkblue")+theme_minimal()+ggtitle("Hàm bậc 4")+
  geom_vline(xintercept = 25, col = "black",linetype=2)
p3<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(method= "lm",formula = y ~ poly(x,degree = 5, raw = TRUE), size = 0.5, color = "darkblue")+theme_minimal()+ggtitle("Hàm bậc 5")+
  geom_vline(xintercept = 25, col = "black",linetype=2)
...
p3<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(data = filter(dat,lstat>15),aes(x = lstat, y = medv),
              method = "lm",formula = y ~ poly(x,degree = 3, raw = TRUE), size = 0.5, color = "darkblue", se = FALSE)+
  geom_smooth(data = filter(dat,lstat<=15),aes(x = lstat, y = medv),
              method = "lm",formula = y ~ poly(x,degree = 3, raw = TRUE), size = 0.5, color = "darkblue", se = FALSE)+
...
```{r fggam03, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Hồi quy giá nhà (medv) theo splines bậc ba của biến giải thích là tỷ lệ người sống dưới mức trung bình (lstat) trong dữ liệu Boston. Hình bên trái: sử dụng một nút tại 15%. Hình bên phải: sử dụng hai nút tại 10% và 20%"}
library(MASS)
library(splines)
dat<-Boston
p1<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(aes(x = lstat, y = medv),
              method = "lm",formula = y ~ splines::bs(x, knots = 15, degree = 3), size = 0.5, color = "darkblue", se = FALSE)+
  theme_minimal()+
  geom_vline(xintercept = 15, col = "black",linetype=2)

p2<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(aes(x = lstat, y = medv),
              method = "lm",formula = y ~ splines::bs(x, knots = c(10,20), degree = 3), size = 0.5, color = "darkblue", se = FALSE)+
  theme_minimal()+
  geom_vline(xintercept = 10, col = "black",linetype=2)+
  geom_vline(xintercept = 20, col = "black",linetype=2)
...
```{r fggam04, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Lựa chọn số nút tối ưu bằng xác thực chéo khi hồi quy giá nhà (medv) theo splines của biến giải thích là tỷ lệ số người có mức sống dưới mức trung bình (lstat). Tham số của xác thực chéo (số folds) được lựa chọn là K = 5. Hình bên trái sử dụng splines bậc hai. Hình bên phải sử dụng splines bậc ba."}
dat<-Boston
fold_number = 5
knot_number = 10
set.seed(10)
...
p1<-dat1%>%ggplot(aes(x,y))+geom_point(shape = 21, col = "#640514", size = 2)+
  geom_point(data = filter(dat1,y == min(dat1$y)),aes(x,y),shape = 21, col = "#640514", fill = "orange", size = 2)+
...
dat1<-data.frame(x = 1:knot_number, y = apply(cv.error,2,mean) )
p2<-dat1%>%ggplot(aes(x,y))+geom_point(shape = 21, col = "#640514", size = 2)+
  geom_line(linetype = 2, color = "grey")+
  geom_point(data = filter(dat1,y == min(dat1$y)),aes(x,y),shape = 21, col = "#640514", fill = "orange", size = 2)+
  geom_point(data = filter(dat1,y == min(dat1$y)),aes(x,y),shape = 21, col = "red", fill = "grey", size = 8, alpha = 0.3)+
  theme_minimal()+
...
p1<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(method = "lm",formula = y ~ bs(x,degree = 3, knots = lstat_knot),
              size = 0.5, color = "darkblue")+ggtitle("Splines")+
...
p2<-dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(method = "lm",formula = y ~ ns(x,knots = lstat_knot[2:4], Boundary.knots = lstat_knot[c(1,5)]),
              size = 0.5, color = "darkblue")+ggtitle("Natural Splines")+
  geom_point(data=dat1,aes(x,y),size=0.8)+
  theme_classic()+geom_vline(xintercept = lstat_knot,color = "grey", linetype=2)
grid.arrange(p1,p2,ncol = 2, nrow = 1)
```
...
```{r fggam06, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="Xây dựng hàm f quá linh hoạt. Sai số trên tập huấn luyện mô hình sẽ nhỏ nhưng sai số trên dữ liệu kiểm tra mô hình sẽ lớn."}
lstat_knot<-as.numeric(quantile(dat$lstat,seq(1/(k+1),k/(k+1),length=k)))
dat%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_smooth(method = "lm",formula = y ~ bs(x,degree = 3, df = 200),
              size = 0.5, color = "darkblue", se = FALSE, alpha = 0.7)+
  theme_minimal()
```
...
```{r fggam07, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Sự linh hoạt của các hàm số so với hàm không linh hoạt là hàm tuyến tính. Hình bên trái: Hàm số ít linh loạt. Hình bên phải: Hàm số rất linh hoạt."}
n<-1000
k<-0.5
...
p1<-dat1%>%ggplot(aes(x,y))+geom_line(color = "#640514")+theme_minimal()+
  labs(title="Hàm ít linh hoạt",
       subtitle=TeX("$\\int f^{''}(t)^2 dt \\approx 500$"))+
  geom_segment(data = data.frame(x = c(-1,1),y=f(c(-1,1))), 
               aes(x = -1, xend = 1, y = f(-1), yend=f(1)), color = "darkblue", linetype=2)+
  ylim(c(-20,20))
...
p2<-dat1%>%ggplot(aes(x,y))+geom_line(color = "#640514")+theme_minimal()+
...
p1<-dat1%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_line(aes(x = lstat, y = sy),size = 0.5, color = "darkblue",alpha = 0.7)+
...
p2<-dat1%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_line(aes(x = lstat, y = sy),size = 0.5, color = "darkblue",alpha = 0.7)+
  labs(title = "Smoothing Splines", 
    subtitle =  paste("Bậc tự do bằng",df1+1))+theme_minimal()
...
p3<-dat1%>%ggplot(aes(x = lstat, y = medv))+geom_point(shape=21,color="#640514", alpha = 0.5)+
  geom_line(aes(x = lstat, y = sy),size = 0.5, color = "darkblue",alpha = 0.7)+
  labs(title = "Smoothing Splines", 
    subtitle =  paste("Bậc tự do bằng",df1+1))+theme_minimal()
...
```{r fggam09a, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5,  fig.cap="Hàm số phi tuyến được xây dựng bằng phương pháp hồi quy cục bộ. Đường màu xanh da trời là giá trị thật của hàm f. Tham số span được sử dụng trong hồi quy cục bộ là 0.3"}
...
dat%>%ggplot(aes(x,y))+geom_point(aes(color = col1),shape=21)+
  scale_color_manual(values = c("#640514","orange"))+
  geom_smooth(method = "loess", span = s0, se = FALSE, color = "orange",linetype = 2)+
  geom_line(aes(x,y1),color= "darkblue", size = 0.5)+
  geom_point(aes(x=x0,y=y0),size=3, color = "orange")+
  geom_segment(aes(x = 0.39,y = 0.5, xend = 0, yend = 2.1), color = "orange", size = 0.5)+
  theme_minimal()+theme(legend.position = "none")+
  ggtitle("Hối quy cục bộ với tham số span bằng 0.3")
```


```{r fggam09b, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5,  fig.cap="Hàm số phi tuyến được xây dựng bằng phương pháp hồi quy cục bộ. Đường màu xanh da trời là giá trị thật của hàm f. Tham số span được sử dụng trong hồi quy cục bộ là 0.7"}
set.seed(10)
n<-200
x<-rnorm(n,1,1)
y<-2*(x-1)^2 + rnorm(n,0,0.5)
...
dat%>%ggplot(aes(x,y))+geom_point(aes(color = col1),shape=21)+
  ...
  theme_minimal()+theme(legend.position = "none")+
  ggtitle("Hồi quy cục bộ với tham số span bằng 0.7")
```
...
```{r fggam10, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="Giá nhà được hồi quy cục bộ theo biến giải thích là tỷ lệ người có thu nhập thấp. Tham số span được lựa chọn bằng xác thực chéo."}
dat<-Boston
fold_number = 10
set.seed(100)
index<-createFolds(dat$medv,k = fold_number, list = TRUE)
...
dat1%>%ggplot(aes(x,y))+geom_point(shape = 21, col = "#640514", size = 2)+
  geom_line(linetype = 2, color = "grey")+
  geom_point(data = filter(dat1,y == min(dat1$y)),aes(x,y),shape = 21, fill = "orange", size = 2, alpha = 0.8)+
  geom_point(data = filter(dat1,y == min(dat1$y)),aes(x,y),shape = 21, col = "red", fill = "grey", size = 8, alpha = 0.3)+
  theme_minimal()+
...
p1<-ggplot(data = data.frame(x = 1:k, y = result[1,]), aes(x,y)) + 
  geom_point(fill = "#640514",shape = 21, size = 2, alpha = 0.3)+geom_line(linetype = 2, col = "#640514")+
  theme_minimal()+ggtitle("Hệ số chặn hàm f1")+ylab("Giá trị")+xlab("Số vòng lặp")+
  scale_x_continuous(breaks = seq(2,k,2)) + ylim(-34,-19)+
  geom_hline(yintercept = result[1,k], col = "darkblue", alpha = 0.7)
...
p2<-ggplot(data = data.frame(x = 1:k, y = result[2,]), aes(x,y)) + 
  ...
  geom_hline(yintercept = result[2,k], col = "darkblue", alpha = 0.7)
...
p3<-ggplot(data = data.frame(x = 1:k, y = result[3,]), aes(x,y)) + 
  ...
  geom_hline(yintercept = result[3,k], col = "darkblue", alpha = 0.7)
...
p4<-ggplot(data = data.frame(x = 1:k, y = result[4,]), aes(x,y)) + 
  ...
  geom_hline(yintercept = result[4,k], col = "darkblue", alpha = 0.7)

grid.arrange(p1,p2,p3,p4, ncol = 2, nrow = 2)
```

Hình \@ref(fig:fggam11) mô tả sự hội tụ của các hệ số chặn và hệ số góc của các hàm $f_1$ và $f_2$ lần lượt là các hàm tuyến tính của biến `r colorize_style("rm")` và biến `r colorize_style("lstat")` trong mô hình hồi quy đa biến với biến mục tiêu là biến `r colorize_style("medv")` trên dữ liệu `r colorize_style("Boston")`. Trước hết bạn đọc có thể thấy rằng các hệ số tuyến tính hội tụ rất nhanh chỉ sau hơn 10 bước lặp. Thứ hai, các hàm ước lượng được từ *backfitting* là
\begin{align}
...
Trong thực tế, bạn đọc không cần phải viết vòng lặp để ước lượng mô hình cộng tính tổng quát mà sử dụng thư viện `r colorize_style("gam")`. Hàm số được sử dụng để xây dựng và ước lượng mô hình cộng tính tổng quát trên R là hàm cùng tên với thư viện, hàm `r colorize_style("gam()")`. Chúng ta sẽ sử dụng hàm số này nhiều hơn trong phần thực hành. 
...
Quay trở lại ví dụ khi chúng ta xây dựng mô hình cộng tính tổng quát khi biến mục tiêu medv phụ thuộc vào hai biến giải thích là `r colorize_style("lstat")` và `r colorize_style("rm")`. Giả sử chúng ta lựa chọn dạng hàm cho `r colorize_style("lstat")` và `r colorize_style("rm")` đều là các *smoothing spline*. Tham số duy nhất của *smoothing spline* là bậc tự do hiệu quả. Tìm bậc tự do hiệu quả cho `r colorize_style("lstat")` và `r colorize_style("rm")` phải thực hiện đồng thời và dựa trên sai số xác thực chéo 

```{r fggam12, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="Sai số xác thực chéo khi lựa chọn tham số bậc tự do hiệu quả cho smoothing spline của biến lstat và bậc tự do hiệu quả cho smoothing spline của biến rm."}
...
mypalette1<-colorRampPalette(c("#640514","#FF7368"), space = "Lab")(10)
...
Hình \@ref(fig:fggam12) mô tả quá trình tìm kiếm tham số bậc tự do hiệu quả cho biến `r colorize_style("lstat")` và tham số bậc tự do hiệu quả cho biến `r colorize_style("rm")` dựa trên sai số xác thực chéo. Có thể thấy rằng tham số cho sai số xác thực chéo nhỏ nhất là $df = 7.9$ đối với biến `r colorize_style("lstat")` và $df = 5.9$ đối với biến `r colorize_style("rm")`. 
...
### Hồi quy spline trên dữ liệu `r colorize_style("Boston")`

### Hồi quy cục bộ trên dữ liệu `r colorize_style("Boston")`

### Mô hình công tính tổng quát trên dữ liệu `r colorize_style("Boston")`
