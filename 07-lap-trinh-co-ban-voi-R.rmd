...
```{r tbbasic01, warning=FALSE, message=FALSE, echo = FALSE}
...
```{r fgbasic01, warning = FALSE, message=FALSE, echo = FALSE, fig.cap="Sự khác nhau về dung lượng bộ nhớ cần sử dụng để lưu véc-tơ kiểu số nguyên và véc-tơ kiểu số thập phân."}
k<-100
...
```{r tbbasic02, warning=FALSE, message=FALSE, echo = FALSE}
...
kable(table1, booktabs = T,
      col.names = c("Ký hiệu",
        "Phép tính"),
      caption = "Các phép toán cơ bản thường được sử dụng với biến kiểu số.",
      escape=F) %>%
  column_spec(c(2,2)) %>%
  kable_styling(latex_options = "scale_down")
```
...
```{r tbbasic03, warning=FALSE, message=FALSE, echo = FALSE}
...
kable(table2, booktabs = T,
      col.names = c("Loại số",
        "Ý nghĩa"),
      caption = "Các giá trị số được viết kiểu khoa học và các giá trị số đặc biệt",
      escape=F) %>%
  column_spec(c(2,2)) %>%
  kable_styling(latex_options = "scale_down")
```
...
```{r tbbasic04, warning=FALSE, message=FALSE, echo = FALSE}
Comp <- c(" < "," > "," <= "," >= "," == "," != ")
...
kable(table3, booktabs = T,
      col.names = c("Phép so sánh",
        "Ý nghĩa"),
      caption = "Các phép so sánh cơ bản cho kết quả là một biến logic",
      escape=F) %>%
  column_spec(c(2,2)) %>%
  kable_styling(latex_options = "scale_down")
```
...
```{r tbbasic05, warning=FALSE, message=FALSE, echo = FALSE}
Comp <- c("&","$|$","$!$")
...
kable(table4, booktabs = T,
      col.names = c("Toán tử logic",
        "Ý nghĩa"),
      caption = "Các toán tử sử dụng cùng với biến kiểu logic",
      escape=F) %>%
  column_spec(c(2,2)) %>%
  kable_styling(latex_options = "scale_down")
```
...
```{r tbbasic06, warning=FALSE, message=FALSE, echo = FALSE}
...
kable(table5, booktabs = T,
      col.names = c("Kết hợp",
        "Kết quả"),
      caption = "Các biến logic được kết hợp bằng các toán tử logic",
      escape=F) %>%
  column_spec(c(2,2)) %>%
  kable_styling(latex_options = "scale_down")
```
...
```{r tbbasic07, warning=FALSE, message=FALSE, echo = FALSE}
...
kable(table6, booktabs = T,
      col.names = c("Hàm số",
        "Ý nghĩa"),
      caption = "Các hàm thường sử dụng để xử lý biến kiểu chuỗi ký tự",
      escape=F) %>%
  column_spec(c(2,2)) %>%
  kable_styling(latex_options = "scale_down")
```
...
```{r tbbasic08, warning=FALSE, message=FALSE, echo = FALSE}
...
```{r tbbasic09, warning=FALSE, message=FALSE, echo = FALSE}
...
```{r tbbasic10, warning=FALSE, message=FALSE, echo = FALSE}
...
```{r tbbasic11, warning=FALSE, message=FALSE, echo = FALSE}
Col1 <- c("next", "break")
...
p1<-df%>%ggplot()+
  ...
  theme_minimal()+
  xlab("")+ylab("")+
  geom_polygon(data = df1, aes(x=x,y=y),alpha=0.5,fill="grey")+
...
```{r gradientdescent02, warning = FALSE, message=FALSE, echo = FALSE, fig.cap = "Quá trình hội tụ các điểm ngẫu nhiên đến điểm cực tiểu của hàm f sử dụng thuật toán gradient descent"}
lambda<-0.1
vong_lap_toi_da = 40
sai_so_nho_nhat<-10^(-8)
...
expand.grid(X1 = seq(-50,50,length=200), X2 = seq(-50,50,length=200)) %>%
  ...
  ggplot() +
  geom_contour(aes(X1, X2, z = Z),bins = 20,alpha = 0.2,color = "black", linetype=2)+
  xlim(-50,50)+ylim(-50,50)+
  geom_path(data = dat, aes(x = x1, y = x2, group = loop),alpha = 0.3)+
  geom_point(data = dat, aes(x =x1, y = x2, alpha = value/10^4))+
  geom_point(data = dplyr::filter(dat, so_vong_lap == 1), aes(x =x1, y = x2),col="blue",size = 1.5)+
  geom_point(data = data.frame(x = 5, y = 10), aes(x,y), fill = "orange",shape = 21, size = 2, alpha = 0.5)+
