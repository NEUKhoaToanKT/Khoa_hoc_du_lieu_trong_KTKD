...
1. Quảng cáo qua truyền hình: biến `r colorize_style("TV")`;

2. Quảng cáo qua các nền tảng mạng xã hội: biến `r colorize_style("Social_Media")`; và, 

3. Quảng cáo qua phát tờ rơi: biến `r colorize_style("Flyer")`. 

Hình \@ref(fig:fglm001) mô tả mối liên hệ giữa doanh thu bán hàng, biến `r colorize_style("Sales")`, với chi phí thực hiện các phương thức quảng cáo ở 55 cửa hàng. Lưu ý rằng doanh thu bán hàng được tính bằng đơn vị tỷ đồng trong khi các chi phí quảng cáo được tính bằng đơn vị triệu đồng.

```{r fglm001, warning=FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap= "Doanh thu bán hàng (tỷ đồng) và mối liên hệ với chi phí quảng cáo. Hình bên trái: Chi phí quảng cáo trên Tivi. Hình ở giữa: chi phí quảng cáo qua các nền tảng mạng xã hội. Hình bên phải: quảng cáo theo hình thức phát tờ rơi"}
n<-55
set.seed(10)
M1<-matrix(1,3,3)
...
p1<-dat%>%ggplot(aes(y=Sales,x=TV))+geom_point(shape=21,color="#640514")+
  geom_smooth(method="lm", se=FALSE, size = 0.5, color = "darkblue")+
  theme_classic()+ylab("Doanh thu bán hàng (tỷ đồng)")+xlab("Quảng cáo TV (triệu)")
p2<-dat%>%ggplot(aes(y=Sales,x=Social_Media))+geom_point(shape=21,color="#640514")+
  geom_smooth(method="lm", se=FALSE, size = 0.5, color = "darkblue", )+
  theme_classic()+ylab("Doanh thu bán hàng (tỷ đồng)")+xlab("Qc mạng xã hội (triệu)")
p3<-dat%>%ggplot(aes(y=Sales,x=Flyer))+geom_point(shape=21,color="#640514")+
  geom_smooth(method="lm", se=FALSE, size = 0.5, color = "darkblue")+
...
```{r fglm01, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.cap="Phương pháp bình phương nhỏ nhất được sử dụng để ước lượng hệ số cho mô hình hồi quy đơn biến doanh thu bán hàng theo chi phí quảng cáo trên truyền hình. Đường hồi quy tuyến tính được ước lượng bằng phương pháp bình phương nhỏ nhất là đường thẳng có tổng bình phương khoảng cách các điểm dữ liệu đến đường thẳng này là nhỏ nhất", fig.retina=5}
...
xi<-dat$TV[34]
yi.hat<-b0 + b1 * xi
yi<-dat$Sales[which(dat$TV==xi)]

dat%>%ggplot(aes(x=TV,y=Sales))+geom_point(size=2,shape = 21, color="#640514")+
  geom_smooth(method="lm", se = FALSE, col = "darkblue", size = 0.5)+
  geom_segment(aes(x = TV, y = Sales, xend = TV, yend = y.hat), alpha = 0.8, linetype = 3, col = "gray")+
  
  geom_point(data = data.frame(x = c(xi),y = c(yi)), aes(x,y), size=2,shape = 21, color="#640514",fill = "orange")+
  geom_text(data = data.frame(x = c(xi),y = c(yi)), aes(x,y), label = TeX("$(\\x_i,\\y_i)$"),nudge_y = -0.03)+
  
  geom_point(data = data.frame(x = c(xi),y = c(yi.hat)), aes(x,y), size=2, shape = 4, color="orange")+
  geom_text(data = data.frame(x = c(xi),y = c(yi.hat)), aes(x,y), label = TeX("$(\\x_i,\\hat{y}_i)$"), nudge_y = 0.04)+
  
  geom_text(data = data.frame(x = c(xi),y = c((yi+yi.hat)/2)), aes(x,y), label = TeX("$\\e_i$"), nudge_x = 1.3)+
  
  geom_segment(aes(x = xi, y = yi, xend = xi, yend = yi.hat), alpha = 0.8, linetype = 4, col = "orange")+
  
  geom_text(data = data.frame(x = c(137),y = c(8.4)), aes(x,y), label = TeX("$y = \\hat{\\beta}_0 + \\hat{\\beta}_1 \\times x$"),nudge_y = 0.1, size = 4)+
  geom_segment(aes(x = 137, y = 8.465, xend = 138, yend = 8.423), linetype = 1, col = "gray", size = 0.3)+
  
  theme_classic()+ ylab("Doanh thu bán hàng (tỷ đồng)") + xlab("Chi phí quảng cáo trên truyền hình (triệu đồng)")
```

Hình \@ref(fig:fglm01) mô tả đường hồi quy tuyến tính đơn được xây dựng trên dữ liệu về quảng cáo với biến phụ thuộc là doanh số bán hàng và biến độc lập là chi phí quảng cáo trên truyền hình, với $\hat{\beta}_0$ = `r round(b0,3)` và $\hat{\beta}_1$ = `r round(b1,3)`. 
...
Để thực hiện ước lượng mô hình tuyến tính trong R, chúng ta sử dụng hàm `r colorize_style("lm()")` của thư viện `r colorize_style("stat")`. Câu lệnh thực hiện ước lượng mô hình tuyến tính như sau
...
Từ kết quả của mô hình, chúng ta có một ước lượng cho mô hình tuyến tính đơn giản mà biến `r colorize_style("Sales")` phụ thuộc vào biến `r colorize_style("TV")` như sau:
\begin{align}
\text{Sales} = `r round(b0,5)`  + `r round(b1,5)`  \cdot \text{TV}
(\#eq:lm02a)
\end{align}

Theo như mô hình ước lượng được, khi thêm 1 triệu đồng chi cho quảng cáo trên truyền hình dẫn đến việc doanh thu bán hàng trung bình tăng thêm khoảng `r round(b1*1000,1)` triệu đồng. Doanh thu bán hàng trung bình tăng thêm `r round(b1*1000,1)` triệu có thể hiểu là có cửa hàng có doanh thu tăng nhiều hơn, có cửa hàng có doanh thu tăng ít hơn, nhưng khi lấy giá trị trung bình phần doanh thu tăng thêm của các cửa hàng sẽ có kết quả là khoảng `r round(b1*1000,1)` triệu đồng.
...
```{r fglm02, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="Hình bên trái: Đường hồi quy tuyến tính thật màu đen và đường hồi quy tuyến tính ước lượng màu đỏ. Hình bên phải: Đường hồi quy tuyến tính thật màu đen và các đường hồi quy tuyến tính ước lượng màu xanh"}
set.seed(10)
n<-100
x1<-rnorm(n,0,1)
epsilon<-rnorm(n,0,2)
...
p1<-dat1%>%ggplot(aes(x=x,y=y))+geom_point(size=2,shape=21)+
  ...
  geom_abline(slope = 2, intercept = 1, col = "black")+
  theme_classic()
...
p2<-p2+geom_abline(slope = 2, intercept = 1, col = "black")
...
```{r tblm01, warning = FALSE, message=FALSE, echo = FALSE, tab.cap="Các hệ số ước lượng trong mô hình hồi quy đơn cho dữ liệu Quảng cáo" }
kable(df, booktabs = T, "html",
      col.names = c("Hệ số",
...
```{r tblm03a, echo = FALSE, tab.cap= "Doanh thu bán sản phẩm phụ thuộc vào chi phí quảng cáo trên truyền hình"}
kable(df_TV, booktabs = T, "html",
...
```{r tblm03b, warning=FALSE, message=FALSE, echo = FALSE, tab.cap= "Doanh thu bán sản phẩm phụ thuộc vào chi phí quảng cáo trên mạng xã hội"}
kable(df_SM, booktabs = T, "html",
      col.names = c("Ước lượng","Độ lệch chuẩn", "Thống kê t", "p-value" ),
      escape = F) %>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```
...
```{r tblm03c, warning=FALSE, message=FALSE, echo = FALSE, tab.cap= "Doanh thu bán sản phẩm phụ thuộc vào chi phí quảng cáo tờ rơi"}
kable(df_FL, booktabs = T, "html",
      col.names = c("Ước lượng","Độ lệch chuẩn", "Thống kê t", "p-value" )) %>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```
...
(\#eq:lm101)
\end{align}
trong đó `r colorize_style("Sales")` là doanh thu từ bán hàng của 55 cửa hàng, `r colorize_style("TV")` là chi phí quảng cáo trên truyền hình, `r colorize_style("Social_Media")` là chi phí quảng cáo qua mạng xã hội, và `r colorize_style("Flyer")` là chi phí quảng cáo qua tờ rơi.
...
Bạn đọc có thể sử dụng hàm `r colorize_style("lm()")` để thực hiện ước lượng các hệ số tuyến tính. Chúng ta thực hiện ước lượng mô hình đa biến cho dữ liệu Quảng cáo như sau
```{r, warning=FALSE, message=FALSE, eval = FALSE}
...
```{r tblm04, echo = FALSE, tab.cap="Các hệ số ước lượng trong mô hình hồi quy đa biến cho dữ liệu Quảng cáo"}
kable(df, booktabs = T, "html",
      col.names = c("Ước lượng","Độ lệch chuẩn", "Thống kê t", "p-value" )) %>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```
...
```{r fglm03, warning = FALSE, message=FALSE, echo = FALSE, fig.cap="Hàm mật độ của phân phối F(3,51) cho dữ liệu quảng cáo. Giá trị thống kê F (F - value) đủ lớn để bác bỏ giả thuyết $H_0$", fig.align='center', fig.retina=5}
dat0<-data.frame(x = seq(0,20,length = 500))
dat0<-mutate(dat0, y = df(x, df1 = 3,  df2 = (55-3-1)))
dat0%>%ggplot(aes(x,y))+
  geom_line(color = "#640514")+
  geom_segment(aes(x = 18.7, yend = 0, xend = 18.7, y = 0.03), color = "darkblue", size = 1, 
...
```{r tblm06, warning=FALSE, message=FALSE, echo = FALSE, tab.cap="Sai số phần dư của các mô hình tuyến tính trên dữ liệu Quảng cáo"}
kable(df, booktabs = T, "html",
      col.names = c("Tổ hợp biến","RSE"))%>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```

Trong các mô hình đơn biến, có thể thấy rằng mô hình sử dụng biến `r colorize_style("Flyer")` làm biến giải thích có *RSE* thậm chí còn nhỏ hơn so với mô hình chỉ sử dụng biến `r colorize_style("TV")`. Khi sử dụng hai biến để giải thích biến doanh thu, bạn đọc có thể nhận thấy rằng mô hình sử dụng `r colorize_style("TV")` và `r colorize_style("Social_Media")` có *RSE* nhỏ hơn hẳn so với các mô hình còn lại. Khi thêm biến `r colorize_style("Flyer")` vào mô hình đã bao gồm `r colorize_style("TV")` và `r colorize_style("Social_Media")`, *RSE* gần như không thay đổi. Không giống như hệ số *R-squared* luôn tăng khi thêm biến vào mô hình, các mô hình có nhiều biến hơn có thể có *RSE* cao hơn nếu mức giảm *RSS* nhỏ hơn so với sự gia tăng số lượng biến. 
...
Khi ước lượng hệ số tuyến tính bằng phương pháp bình phương nhỏ nhất, chúng ta luôn giả định rằng tất cả các biến trong mô hình đều là biến định lượng. Trong thực tế chúng ta lại rất hay thường gặp các biến giải thích là kiểu biến định tính. Ví dụ: dữ liệu có tên `r colorize_style("exposure.csv")` là dữ liệu về yêu cầu bồi thường viện phí và tổng số tiền bồi thường của khác hàng tham gia bảo hiểm sức khỏe tự nguyện tại một công ty bảo hiểm. Chúng tôi để mô hình ở dạng đơn giản nhất khi chỉ có hai biến phụ thuộc là giới tính, biến `r colorize_style("Gender")`, và độ tuổi, biến `r colorize_style("Age")`, của người được thanh toán bảo hiểm sức khỏe. Biến mục tiêu trong mô hình sẽ là số tiền bồi thường trung bình của những khách hàng, biến `r colorize_style("Ave_Claim")`, được tính bằng tổng số tiền của tất cả các lần bồi thường chia cho tổng số lần khách hàng gửi yêu cầu.
...
Giả sử chúng ta muốn xây dựng mô hình tuyến tính mà biến mục tiêu là số tiền bồi thường trung bình của khách hàng và các biến giải thích là độ tuổi và giới tính của khách hàng. Biến `r colorize_style("Age")` có thể coi là biến định lượng và có thể nhận giá trị là mọi số tự nhiên từ 18 tuổi đến 65 tuổi. Biến `r colorize_style("Gender")` không thể được sử dụng như một biến định lượng bởi vì chúng ta không thể đưa ra các so sánh định lượng giữa hai giá trị mà biến giới tính nhận.

```{r fglm04, warning = FALSE, message=FALSE, echo = FALSE, fig.cap="Hình bên trái: Mối liên hệ giữa số tiền bồi thường trung bình với độ tuổi của người được bảo hiểm; Hình bên phải: Mối liên hệ giữa số tiền bồi thường trung bình với giới tính của người được bảo hiểm", fig.align='center', fig.retina=5}

p1<-dat%>%ggplot(aes(y=Ave_Claim,x=Age))+geom_point(shape=21,color="#640514",alpha=0.5)+
  geom_smooth(se=FALSE, size = 0.5, color = "darkblue")+ylim(c(0,100))+
  theme_classic()+ylab("Số tiền bồi thường trung bình (triệu đồng)")+xlab("Tuổi của người được bảo hiểm")
p2<-dat%>%ggplot(aes(y=Ave_Claim,x=as.factor(Gender),fill = Gender))+geom_boxplot(color="#640514",alpha=0.5)+
...
```{r tblm07, warning=FALSE, message=FALSE, echo = FALSE, tab.cap= "Các hệ số ước lượng trong mô hình hồi quy tuyến tính với biến định tính Gender nhận một trong hai giá trị là Male hoặc Female"}
kable(df, booktabs = T, "html",
      col.names = c("Ước lượng","Độ lệch chuẩn", "Thống kê t", "p-value" )) %>% 
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```

Hệ số ước lượng của mô hình tuyến tính với biến phụ thuộc định tính được trình bày trong bảng \@ref(tab:tblm07). Bạn đọc thấy rằng các hệ số ước lượng được đều có ý nghĩa thống kê vì giá trị *p-value* đều rất nhỏ. Hệ số tuyến tính của biến độ tuổi bằng `r round(df[2,1],2)` cho thấy rằng nếu tuổi của người được bảo hiểm tăng thêm 1 tuổi, thì số tiền bồi thường trung bình sẽ tăng khoảng `r round(df[2,1],2)` triệu đồng. Hệ số tuyến tính của biến `r colorize_style("Gender-Nữ")` là số âm cho biết cùng một độ tuổi, trung bình mỗi lần yêu cầu bồi thường nữ giới sẽ có số tiền yêu cầu ít hơn nữ giới khoảng `r abs(round(df[3,1],2))` triệu đồng. Việc lựa chọn mã hóa giới tính trong phương trình \@ref(eq:lm109) là hoàn toàn tự do và không ảnh hưởng đến kết quả của mô hình hồi quy. Nếu bạn đọc sử dụng cách mã hóa nữ giới là 0 và nam giới là 1, kết quả thu được sẽ có hệ số của biến `r colorize_style("Gender-Nam")` là số dương, có giá trị bằng với giá trị tuyệt đối của hệ số của biến `r colorize_style("Gender-Nữ")` trong bảng \@ref(tab:tblm07).
...
```{r tblm08,  echo = FALSE,tab.cap="Các hệ số ước lượng trong mô hình hồi quy quy đa thức với bình phương của biến Age là biến giải thích"}
kable(df, booktabs = T, "html",
      col.names = c("Ước lượng","Độ lệch chuẩn", "Thống kê t", "p-value" )) %>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```
...
Trong nhiều trường hợp, biến mục tiêu không chỉ phụ thuộc vào từng biến giải thích một cách riêng lẻ, mà còn phụ thuộc vào sự tương tác giữa các biến giải thích. Một ví dụ điển hình cho trường hợp này là khi sử dụng mô hình tuyến tính để mô tả mối liên hệ giữa số lượng thành phẩm của một nhà máy sản xuất, biến `r colorize_style("products")`, với số lượng công nhân, biến `r colorize_style("workers")`, và số lượng máy chế tạo, biến `r colorize_style("machines")`. Nhìn chung khi tăng số lượng công nhân hoặc tăng số lượng máy thì số lượng thành phẩm sẽ tăng lên. Tuy nhiên mô hình hồi quy tuyến tính chỉ bao gồm hai biến giải thích `r colorize_style("workers")` và `r colorize_style("machines")` sẽ không mô tả được thực tế là khi tăng số lượng công nhân lên quá nhiều sẽ dẫn đến việc công nhân không có máy để sản xuất nên số lượng thành phẩm cũng sẽ không tăng theo tương ứng. Chính vì thế, để mô tả được thực tế đó, mô hình tuyến tính cần có biến giải thích mô tả sự tương tác giữa `r colorize_style("workers")` và `r colorize_style("machines")`:
\begin{align}
\text{products} = \beta_0 + \beta_1 \cdot \text{workers}  + \beta_2 \cdot \text{machines} + \beta_3 \cdot \text{workers} \times \text{machines} + \epsilon
(\#eq:lm112)
\end{align}
hoặc chúng ta cũng có thể viết mô hình \@ref(eq:lm112) dưới dạng mô hình tuyến tính mà hệ số tuyến tính của biến `r colorize_style("machines")` phụ thuộc vào biến `r colorize_style("workers")`
\begin{align}
...
p1<-dat%>%ggplot(aes(x=Ave_Claim,y=e1))+geom_point(shape=21,color="#640514",alpha=0.5)+
  geom_smooth(se=FALSE, color = "darkblue",size = 0.7)+xlim(c(0,30))+ylim(c(-30,30))+
  theme_classic()+xlab("Số tiền bồi thường trung bình (triệu đồng)")+ylab("Phần dư")+
  geom_abline(slope = 0, intercept = 0, size = 0.5, color = "grey30") + ggtitle("Mô hình tuyến tính thông thường")
p2<-dat%>%ggplot(aes(x=Ave_Claim,y=e2))+geom_point(shape=21,color="#640514",alpha=0.5)+
  geom_smooth(se=FALSE, color = "darkblue",size = 0.7)+xlim(c(0,30))+ylim(c(-30,30))+
  theme_classic()+xlab("Số tiền bồi thường trung bình (triệu đồng)")+ylab("Phần dư")+
  geom_abline(slope = 0, intercept = 0, color = "grey30") + ggtitle("Mô hình hồi quy đa thức")
grid.arrange(p1,p2,nrow= 1 , ncol = 2)
```
...
```{r fglm06, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="Đồ thị mô tả phần dư có tương quan và không có tương quan. Hình trên: các phần dư có tương quan bằng 0. Hình ở giữa: hai giá trị phần dư cạnh nhau có tương quan 0.5. Hình dưới: hai giá trị phần dư cạnh nhau có tương quan 0.9"}
n<-200
set.seed(50)
rho<-c(0,0.5,0.9)
ep<-matrix(0,3,n)
...
p1<-dat1%>%ggplot(aes(time,res1)) + 
  geom_point(shape=21,color="#640514",alpha=0.5, size = 0.5) + 
  geom_line(size = 0.5,color="#640514") + geom_abline(slope = 0, intercept = 0, color = "grey30")+
...
```{r fglm07, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5,  fig.cap="Đồ thị mô tả phần dư có phương sai thay đổi. Hình ở trên: cho thấy phần dư có phương sai thay đổi. Hình ở dưới: Phần dư có phương sai ổn định"}
set.seed(10)
n<-200
x<-runif(n,0,10)
...
p1<-dat1%>%ggplot(aes(x,y)) + 
  geom_point(shape = 21, color="#640514",alpha=0.5)+geom_abline(slope = 0, intercept = 0, color = "grey30")+
  theme_minimal()+xlab("Ước lượng cho biến mục tiêu")+ylab("Phần dư")+ggtitle("Biến mục tiêu Y")

lm1<-lm(log(y)~x)
dat1<-data.frame(x = lm1$fitted.values, y =  lm1$residuals)
p2<-dat1%>%ggplot(aes(x,y)) + 
  geom_point(shape = 21, color="#640514",alpha=0.5) + ylim(-2,2)+geom_abline(slope = 0, intercept = 0, color = "grey30")+
...
p1<-dat1%>%ggplot()+geom_point(aes(x=X,y=Y,col=Z, fill = Z),shape=21)+
  scale_color_manual(values = c("#640514","orange"))+
  scale_fill_manual(values = c("white","orange"))+
  geom_smooth(aes(x=X,y=Y),method="lm",se = FALSE, size = 0.5, col= "darkblue")+
...
p2<-dat1%>%ggplot()+geom_point(aes(x=X,y=Y1/s_lm_dat1$sigma,col=Z, fill = Z),shape=21)+
  scale_color_manual(values = c("#640514","orange"))+
  scale_fill_manual(values = c("white","orange"))+
  
  geom_abline(slope = 0, intercept = 0, linetype=2, col= "grey30")+
...
#### Đa cộng tuyến
Đa cộng tuyến là sự kiện mà trong đó hai hoặc nhiều biến giải thích có tương quan cao với nhau với nhau. Để giải thích rõ ràng khái niệm này, chúng ta hãy lấy một ví dụ khi xây dựng mô hình hồi quy tuyến tính trên dữ liệu có tên là `r colorize_style("Credit_Card")`. Đây là dữ liệu về thông tin thẻ tín dụng của các khách hàng tại một ngân hàng với biến mục tiêu là số dư tài khoản, có tên là `r colorize_style("Balance")`, và 10 biến giải thích. Để giải thích về đa cộng tuyến, chúng tôi chỉ sử dụng ba biến giải thích là 1. Độ tuổi của chủ thẻ tín dụng, ký hiệu `r colorize_style("Age")`, 2. Hạn mức thẻ tín dụng, ký hiệu `r colorize_style("Limit")`, và 3. Điểm tín dụng của khách hàng, ký hiệu `r colorize_style("Rating")`. Hiện tượng đa cộng tuyến là hiện tượng mà mô hình có các biến giải thích có mối liên hệ rất chặt chẽ với nhau như được minh họa trên Hình \@ref(fig:fglm09)

```{r fglm09, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Mối liên hệ giữa các biến giải thích trong dữ liệu Credit. Hình bên trái: Không cho thấy có mối tương quan giữa hạn mức tín dụng với tuổi của khách hàng. Hình ở giữa: Tương quan giữa hạn mức tín dụng và điểm tính dụng là rất cao. Hình bên phải: Không có tương quan giữa tuổi của khách hàng với xếp hạng tín dụng"}
dat1<-read.csv("../KHDL_KTKD Final/Dataset/Credit_Card.csv")
dat1<-as.data.frame(dat1)
dat1<-select(dat1,Balance,Limit,Age,Rating)
p1<-dat1%>%ggplot(aes(Limit,Age))+geom_point(shape=21,col="#640514")+
  theme_minimal()+xlab("Hạn mức")+ylab("Tuổi khách hàng")
p2<-dat1%>%ggplot(aes(Limit,Rating))+geom_point(shape=21,col="#640514")+
  theme_minimal()+xlab("Hạn mức")+ylab("Xếp hạng tín dụng")
p3<-dat1%>%ggplot(aes(Age, Rating))+geom_point(shape=21,col="#640514")+
...
```{r tblm09, warning=FALSE, message=FALSE, echo = FALSE, tab.cap="Các hệ số ước lượng trong mô hình hồi quy tuyến tính số dư tài khoản phụ thuộc vào độ tuổi và hạn mức tín dụng"}
...
```{r tblm10, warning=FALSE, message=FALSE, echo = FALSE, tab.cap= "Các hệ số ước lượng trong mô hình hồi quy tuyến tính số dư tài khoản phụ thuộc vào hạn mức tín dụng và điểm tín dụng"}
kable(df, booktabs = T, "html",
      col.names = c("Ước lượng","Độ lệch chuẩn", "Thống kê t", "p-value" )) %>%
  kable_styling(bootstrap_options = c("bordered", "hover"), full_width = F)
```

Bảng \@ref(tab:tblm09) và \@ref(tab:tblm10) so sánh các hệ số tuyến tính ước lượng được được từ hai mô hình hồi quy riêng biệt. Trước tiên là hồi quy số dư tài khoản thẻ tín dụng theo độ tuổi và hạn mức tín dụng, sau đó là hồi quy số dư tài khoản theo hạn mức tín dụng và điểm tín dụng. Trong mô hình hồi quy đầu tiên, độ tuổi và hạn mức tín dụng đều có ý nghĩa do giá trị *p-value* rất nhỏ. Trong mô hình thứ hai, hiện tượng đa cộng tuyến giữa hạn mức tín dụng và điểm tín dụng đã khiến độ lệch chuẩn của hệ số ước lượng của biến hạn mức tín dụng tăng lên gấp `r round(df[2,2]/df1[3,2],0)` lần và *p-value* tăng lên thành `r round(df[2,4],3)`. Nói cách khác, sự quan trọng của biến hạn mức tín dụng đã bị che khuất do hiện tượng đa cộng tuyến. Để tránh rơi vào tình trạng như vậy, cần xác định và giải quyết vấn đề đa cộng tuyến trước khi ước lượng mô hình. 
...
```{r tblm11,  echo = FALSE, tab.cap="Hệ số lạm phát phương sai (VIF) của các biến giải thích trong dữ liệu về thẻ tín dụng"}
kable(df, booktabs = T, "html",
...
Dữ liệu được sử dụng để xây dựng mô hình tuyến tính là dữ liệu `r colorize_style("Boston")` trong thư viện `r colorize_style("MASS")`. Đây là dữ liệu về giá nhà tại 506 vùng ngoại ô tại Boston. Biến mục tiêu trong xây dựng mô hình là biến `r colorize_style("medv")` là giá trị trung vị của giá nhà tại vùng đó, đơn vị là nghìn USD. Mô hình có 13 biến phụ thuộc trong đó có 11 biến kiểu số và 2 biến kiểu rời rạc là  `r colorize_style("rad")` và `r colorize_style("chas")`. Mục tiêu của chúng ta là lựa chọn được $k$ biến trong số 13 biến phụ thuộc sao cho *RSE* là nhỏ nhất. 

```{r fglm10, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Phương pháp lựa chọn tập hợp con tốt nhất được thực hiện trên dữ liệu Boston. Đường ranh giới dưới là các mô hình có tổng bình phương phần dư nhỏ nhất"}
library(MASS)
dat<-Boston
...
ggplot()+
  geom_point(data=dat2,aes(Number.Variables,RSS),alpha = 0.2, shape = 21, fill = "#640514")+
  geom_point(data=filter(dat2,Best.Model),aes(Number.Variables,RSS), shape = 21,size =2, fill = "orange")+
  geom_line(data=filter(dat2,Best.Model), aes(Number.Variables,RSS), col = "orange")+
...
Hình vẽ \@ref(fig:fglm11) mô tả các mô hình có $k$ biến tốt nhất với $k = 0, 1, \cdots, 13$ sử dụng phương pháp *forward stepwise selection*. Trong trường hợp dữ liệu `r colorize_style("Boston")`, ba phương pháp lựa chọn biến chúng tôi mô tả ở trên cho cùng một kết quả!

```{r fglm11, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Phương pháp forward stepwise selection được thực hiện trên dữ liệu Boston. Đường ranh giới dưới là các mô hình có sai số trung bình nhỏ nhất."}
## forward 
...
ggplot()+
  geom_point(data=dat2,aes(Number.Variables,RSS),alpha = 0.2, shape = 21, fill = "#640514")+
  geom_point(data=dplyr::filter(dat2,Best.Model),aes(Number.Variables,RSS), shape = 21,size =2, fill = "orange")+
  geom_line(data=dplyr::filter(dat2,Best.Model), aes(Number.Variables,RSS), col = "orange")+
  theme_minimal()+
  theme(legend.position = "none")+
...
```{r fglm111, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="hồi quy *ridge* tương đương với bài toán tìm RSS nhỏ nhất với ràng buộc các hệ số tuyến tính."}
## Mô phỏng dữ liệu 
n<-500
rho<-0.6
sigma<-0.4
...
data.frame(x=0,y=0)%>%ggplot(aes(x,y))+
  geom_polygon(data=hinhtron, aes(x,y), color="black", fill="#640514", alpha = 0.2)+
  ...
  geom_path(data = dat1, aes(x,y),linetype = 3, col = "darkblue")+
  geom_path(data = dat2, aes(x,y),linetype = 3 , col = "darkblue")+
  geom_path(data = dat3, aes(x,y),linetype = 3 , col = "darkblue")+
  expand_limits(x = 0, y = 0)+
  xlim(c(-1,3.2)) + ylim(c(-2.2,1))+
  theme_classic()+
  geom_hline(yintercept = 0, col = "grey30")+geom_vline(xintercept = 0, col = "grey30")+
  geom_point(aes(x=0.95,y=-0.29),fill="#640514",shape=21)+
  geom_segment(aes(x=beta[1]-0.02,y=beta[2]+0.02, xend = 0.98,yend = -0.32), col = "#640514",linetype=2,arrow = arrow(length = unit(0.3, "cm")))+
...
```{r fglm112, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina=5, fig.cap="Hồi quy ridge tương đương với bài toán tìm RSS nhỏ nhất với ràng buộc các hệ số tuyến tính."}
# Vẽ hình lasso
# Tạo dữ liệu để vẽ hình tròn và các elip
...
data.frame(x=0,y=0)%>%ggplot(aes(x,y))+
  ...
  theme_classic()+
  geom_hline(yintercept = 0, col = "grey30")+geom_vline(xintercept = 0, col = "grey30")+
  geom_point(aes(x=0.95,y=-0.06),fill="#640514",shape=21)+
  geom_segment(aes(x=beta[1]-0.02,y=beta[2]+0.02, xend = 0.98,yend = -0.1), col = "#640514",linetype=2,arrow = arrow(length = unit(0.3, "cm")))+
  geom_label(aes(x=beta[1],y=beta[2]),label = "Điểm tối ưu OLS", vjust = 1.3)+
...
Chúng ta sẽ thực hành xây dựng mô hình tuyến tính trên dữ liệu `r colorize_style("Boston")` trong thư viện `r colorize_style("MASS")` là dữ liệu về giá nhà tại các vùng ngoại ô của thành phố Boston. Biến mục tiêu có tên là `r colorize_style("medv")` là giá trị trung vị của giá nhà tại mỗi vùng. Có 13 biến giải thích bao gồm 11 biến giải thích là các biến liên tục và hai biến giải thích rời rạc bao gồm

* `r colorize_style("chas")` nhận hai giá trị là 1 nếu vùng ngoại ô nằm trên đường bờ sông và nhận giá trị bằng 0 nếu vùng đó không nằm trên đường bờ sông. 
* `r colorize_style("rad")` là chỉ số cho biết khả năng tiếp cận đường cao tốc của mỗi vùng. Chỉ số này càng cao thì vùng càng có khả năng tiếp cận đường cao tốc.
...
Trước hết chúng ta sẽ xây dựng mô hình tuyến tính đơn biến mà giá nhà phụ thuộc vào một biến có tên là `r colorize_style("lstat")`. Các hệ số tuyến tính trong mô hình hồi quy đơn $Y \sim \beta_0 + \beta_1 \cdot X$ được ước lượng bằng phương pháp bình phương nhỏ nhất được tính toán như sau:
\begin{align}
...
Như vậy, chúng ta có mô hình tuyến tính đơn biến mà `r colorize_style("medv")` phụ thuộc vào `r colorize_style("lstat")`
\begin{align}
\hat{medv} = 34.5538409 - 0.9500494 \times \text{lstat}
\end{align}

Kết quả mô hình cho thấy khi `r colorize_style("lstat")` tăng thêm 1 (%) thì giá nhà trung bình sẽ giảm đi khoảng 0.950 (nghìn USD). Sai số (phần dư) của mô hình là hiệu số giữa giá trị của biến `r colorize_style("medv")` được ước lượng từ mô hình và giá trị của `r colorize_style("medv")` trong dữ liệu. Chúng ta có đồ thị phần dư như sau

```{r lmthuchanh2, warning = FALSE, message=FALSE, fig.align='center', fig.retina=5, fig.cap= "Đồ thị phần dư trong mô hình hồi quy đơn medv phụ thuộc vào lstat"}
...
Tất cả các tính toán ở trên đều có thể được thực hiện thông qua hàm có sẵn là `r colorize_style("lm()")`. Cách sử dụng hàm `r colorize_style("lm()")` xây dựng mô hình tuyến tính mà biến `r colorize_style("medv")` phụ thuộc vào biến `r colorize_style("lstat")` như sau
```{r lmthuchanh5, warning = FALSE, message=FALSE}
lm1<-lm(medv~lstat, data = Boston)
summary(lm1)
```

Bạn đọc có thể kiểm tra các tính toán ở trên so với kết quả ước lượng từ hàm `r colorize_style("lm()")` là hoàn toàn tương tự nhau. Đối tượng `r colorize_style("lm1")` là một list có 12 phần tử trong đó có các phần tử như `r colorize_style("coefficient")` chứa giá trị các hệ số ước lượng hay `r colorize_style("residuals")` là véc-tơ phần dư.

Mô hình tuyến tính đa biến cũng có thể được ước lượng bằng hàm `r colorize_style("lm()")` giống như mô hình đơn biến. Chúng ta xây dựng mô hình tuyến tính đa biến mà trong đó biến `r colorize_style("medv")` phụ thuộc vào tất cả các biến còn lại như sau
...
Để tính sai số xác thực chéo, với mỗi $i \in \{1, 2, 3, 4, 5\}$ chúng ta xây dựng mô hình trên dữ liệu `r colorize_style("Boston")` loại trừ đi các quan sát thuộc tập xác thực $i$. Sau đó tính toán sai số của mô hình trên tập xác thực thứ $i$. Sai số trong bài toán hồi quy được tính bằng RMSE. Sai số xác thực chéo là giá trị trung bình của sai số trên các tập xác thực $i$, $1 \leq i \leq 5$. Ví dụ, chúng ta xác định sai số của mô hình đơn biến gồm một biến giải thích `r colorize_style("lstat")` và mô hình tuyến tính đa biến bao gồm tất cả các biến như sau:
...
Hồi quy *ridge* và lasso được thực hiện bằng hàm số `r colorize_style("glmnet()")` của thư viện cùng tên `r colorize_style("glmnet")`. Tham số $\lambda$ của các phương pháp này được tìm kiếm bằng phương pháp xác thực chéo. Hàm số thực hiện tính toán sai số xác thực chéo của hồi quy *ridge* và lasso là hàm `r colorize_style("cv.glmnet()")`.
...
Sai số xác thực chéo tính bằng MSE được lưu trong véc-tơ con có tên là `r colorize_style("cvm")`. Chúng ta có sai số xác thực chéo cho mỗi giá trị của $\lambda$ của hồi quy *ridge* và lasso như hình \@ref(eq:fglmthuchanh01)

```{r fglmthuchanh01, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Sai số xác thực chéo tương ứng với mỗi giá trị của siêu tham số lambda trong hồi quy ridge và lasso."}

p1 <- data.frame(x = cv.ridge$lambda, y = cv.ridge$cvm^0.5)%>%
  ggplot(aes(x,y))+geom_point(shape=21,color="#640514",alpha=0.5,size =2)+
...
```{r fglmthuchanh02, warning = FALSE, message=FALSE, echo = FALSE, fig.align='center', fig.retina = 5, fig.cap="Sai số xác thực chéo tương ứng với mỗi giá trị của siêu tham số lambda trong hồi quy ridge và lasso."}
